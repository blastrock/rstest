...
use syn::{parse_quote, visit_mut::VisitMut, FnArg, Ident, ItemFn, Lifetime, Signature};
...
#[derive(Default)]
pub(crate) struct ReplaceAwaitAttribute {
    lifetimes: Vec<Lifetime>,
    errors: Vec<syn::Error>,
}
...
impl VisitMut for ReplaceAwaitAttribute {
    fn visit_fn_arg_mut(&mut self, node: &mut FnArg) {
        let ident = node.maybe_ident().cloned();
        match node {
            FnArg::Typed(t) => {
                ...
                if awaits.is_empty() {
                    return;
                } else if awaits.len() > 1 {
                    self.errors.extend(awaits.iter().skip(1).map(|attr| {
                        syn::Error::new_spanned(
                            attr.into_token_stream(),
                            "Cannot use #[await_future] more than once.".to_owned(),
                        )
                    }));
                    return;
                }
...
#[derive(Default)]
pub(crate) struct RemoveAwaitAttribute {
    await_args: Vec<Ident>,
}

impl RemoveAwaitAttribute {
    pub(crate) fn replace(item_fn: &mut ItemFn) -> Vec<Ident> {
        Self::replace_sig(&mut item_fn.sig)
    }

    pub(crate) fn replace_sig(sig: &mut Signature) -> Vec<Ident> {
        let mut visitor = Self::default();
        visitor.visit_signature_mut(sig);
        visitor.await_args
    }
...
impl VisitMut for RemoveAwaitAttribute {
    fn visit_fn_arg_mut(&mut self, node: &mut FnArg) {
        match node {
            FnArg::Typed(t) => {
                let awaits = extract_arg_attributes(t, |a| attr_is(a, "await_future"));
                ...
                self.await_args.push(node.maybe_ident().unwrap().clone());
            }
            FnArg::Receiver(_) => {}
        }
    }
